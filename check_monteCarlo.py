...
jogos = wd_Chrome.find_elements(By.CSS_SELECTOR, 'div.event__match--twoLine:not(.event__match--scheduled)')
...
for x, link in enumerate(tqdm(id_jogos, total=len(id_jogos))):
    ...
    try:
        ...
        PlacarHome = wd_Chrome.find_element(By.CSS_SELECTOR, 'div.detailScore__wrapper').text.split('-')[0]
        PlacarHome = int(PlacarHome)
        PlacarAway = wd_Chrome.find_element(By.CSS_SELECTOR, 'div.detailScore__wrapper').text.split('-')[1]
        PlacarAway = int(PlacarAway)
        Placar = PlacarHome + PlacarAway
        # Conferir as variáveis com invalid literal for int() with base 10: ''
...
filename = "lista_de_jogos/jogos_do_dia_" + \
    ...
    "_U25FT_com_SD_com_MonteCarlo_com_placar.csv"
